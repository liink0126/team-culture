...
<body>
    <div class="container-wrapper">
        ...
        <div id="survey-container">
            ...
            <div id="surveySection" class="hidden flex flex-col items-center">
                ...
                <button id="showRecommenderButton" class="action-button" disabled>모든 질문에 응답해 주세요</button>
                <div id="survey-error-message" class="mt-4 text-center text-red-500 hidden"></div>
            </div>
        </div>
    ...
    <script type="module">
        ...
        const showRecommenderButton = document.getElementById('showRecommenderButton');
        // Modal elements
        ...
        function updateSurveyUI() {
            if (allQuestionsAnswered()) {
                showRecommenderButton.disabled = false;
                showRecommenderButton.textContent = "결과 분석 및 솔루션 보기";
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Survey -> Recommender
            showRecommenderButton.addEventListener('click', async () => {
                const errorMsg = document.getElementById('survey-error-message');
                errorMsg.classList.add('hidden');

                 if (allQuestionsAnswered()) {
                    showRecommenderButton.textContent = '분석 중...';
                    showRecommenderButton.disabled = true;
                    ...
                    if (!db || !userId) {
                        errorMsg.textContent = '데이터베이스 연결에 실패했습니다. 페이지를 새로고침 후 다시 시도해주세요.';
                    ...
                    try {
                        ...
                        errorMsg.textContent = '오류: 결과를 저장하지 못했습니다. 잠시 후 다시 시도해주세요.';
                        errorMsg.classList.remove('hidden');
                        showRecommenderButton.textContent = '결과 분석 및 솔루션 보기';
                        showRecommenderButton.disabled = false;
                    }
